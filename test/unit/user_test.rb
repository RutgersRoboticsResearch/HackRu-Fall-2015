...
class UserTest < ActiveSupport::TestCase
	...
	test "that creating friendships on a user works" do
		users(:miyamoto).pending_friends << users(:greg)
		users(:miyamoto).pending_friends.reload
	...
	context "#has_blocked?" do
		should "return true if a user has blocked another user" do
			assert users(:miyamoto).has_blocked?(users(:blocked_friend))
		end

	should "return false if a user has not blocked another user" do
		assert !users(:miyamoto).has_blocked?(users(:jon))
	    end
	end
end
