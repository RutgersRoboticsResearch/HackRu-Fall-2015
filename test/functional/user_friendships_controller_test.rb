...
class UserFriendshipsControllerTest < ActionController::TestCase
  context "#index" do
    ...
    context "when logged in" do
      setup do
        @pending_friendship   = create(:pending_user_friendship, user: users(:miyamoto), friend: create(:user, first_name: 'Pending', last_name: 'Friend'))
        @accepted_friendship  = create(:accepted_user_friendship, user: users(:miyamoto), friend: create(:user, first_name: 'Active', last_name: 'Friend'))
        @requested_friendship = create(:requested_user_friendship, user: users(:miyamoto), friend: create(:user, first_name: 'Requested', last_name: 'Friend'))
        @blocked_friendship   = user_friendships(:blocked_by_miyamoto)
  ...
  context "#new" do
    ...
    context "when logged in" do
      ...
      should "assign a new user friendship to the currently logged in user" do
        get :new, friend_id: users(:jon)
        assert_equal users(:miyamoto), assigns(:user_friendship).user
      end
      ...
      should "ask if you really want to friend the user" do
        get :new, friend_id: users(:jon)
        assert_match /Do you really want to friend #{users(:jon).full_name}?/, response.body
      end
    end
  end
    ...
    context "when logged in" do
      ...
      context "successfully" do
        should "create two user friendship objects" do
          assert_difference 'UserFriendship.count', 2 do
            post :create, user_friendship: { friend_id: users(:greg).profile_name }
          end
        end
      end
      ...
      context "with a valid friend_id" do
        ...
        should "assign a friend object" do
          ...
          assert_equal users(:greg), assigns(:friend)
        end

        should "assign a user_friendship object" do
          assert assigns(:user_friendship)
          assert_equal users(:miyamoto), assigns(:user_friendship).user
          assert_equal users(:greg), assigns(:user_friendship).friend
        end
        ...
        should "redirect to the profile page of the friend" do
          assert_response :redirect
          assert_redirected_to profile_path(users(:greg))
        end
  ...
  context "#accept" do
    ...
    context "when logged in" do
      setup do
        @friend = create(:user)
        @user_friendship = create(:pending_user_friendship, user: users(:miyamoto), friend: @friend)
        create(:pending_user_friendship, friend: users(:miyamoto), user: @friend)
        sign_in users(:miyamoto)
  ...
  context "#edit" do
    ...
    context "when logged in" do
      setup do
        @user_friendship = create(:pending_user_friendship, user: users(:miyamoto))
        sign_in users(:miyamoto)
  ...
  context "#destroy" do
    ...
    context "when logged in" do
      setup do
        @friend = create(:user)
        @user_friendship = create(:accepted_user_friendship, friend: @friend, user: users(:miyamoto))
        create(:accepted_user_friendship, friend: users(:miyamoto), user: @friend)
